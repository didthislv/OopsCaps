...
namespace OopsCaps
{
    static class Strings
    {
        ...
        public static string VerAuth(string lang) { return lang == "lv" ? "Versija 1.0\n© 2026 did.this.lv" : (lang == "ru" ? "Версия 1.0\n© 2026 did.this.lv" : "Version 1.0\n© 2026 did.this.lv"); }
    }
    ...
    static class Program
    {
        ...
        internal static Bitmap GenerateOoImage(int size)
        {
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                ...
                using (Brush backBrush = new SolidBrush(Color.Black)) { g.FillEllipse(backBrush, penWidth/2.0f, penWidth/2.0f, size - penWidth, size - penWidth); }
                using (Pen borderPen = new Pen(purpleColor, penWidth)) { g.DrawEllipse(borderPen, penWidth/2.0f, penWidth/2.0f, size - penWidth, size - penWidth); }
        ...
        class BackgroundAppForm : Form
        {
            ...
            public BackgroundAppForm()
            {
                ...
                // Ikonas ģenerēšana pa tiešo atmiņā
            ...
            private void LoadSettings() { string cfg = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "OopsCaps.cfg"); if (File.Exists(cfg)) { try { var lines = File.ReadAllLines(cfg); if (lines.Length >= 7) { uint.TryParse(lines[0], out Mods); uint.TryParse(lines[1], out VkInv); uint.TryParse(lines[2], out VkUpr); uint.TryParse(lines[3], out VkLwr); uint.TryParse(lines[4], out VkTtl); Lang = lines[5].Trim().ToLower(); bool.TryParse(lines[6], out PlaySound); } } catch {} } }
            public void SaveSettings() { try { File.WriteAllLines(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "OopsCaps.cfg"), new string[] { Mods.ToString(), VkInv.ToString(), VkUpr.ToString(), VkLwr.ToString(), VkTtl.ToString(), Lang, PlaySound.ToString() }); RegKeys(); } catch {} }
        ...
        class SettingsForm : Form
        {
            ...
            CheckBox chkCtrl, chkShift, chkAlt, chkAuto, chkSound; ComboBox cmbInv, cmbUpr, cmbLwr, cmbTtl, cmbLang;
            Label lblInv, lblUpr, lblLwr, lblTtl, lblLang, lblSupport, lblVerAuth;

            public SettingsForm(BackgroundAppForm mainApp)
            {
                app = mainApp; this.Size = new Size(420, 510); this.FormBorderStyle = FormBorderStyle.FixedDialog; this.StartPosition = FormStartPosition.CenterScreen;

                grpHotkeys = new GroupBox() { Location = new Point(15, 15), Size = new Size(375, 150) };
                ...
                lblInv = new Label() { Location = new Point(100, 32), AutoSize = true }; cmbInv = CreateCmb(app.VkInv, 200, 30);
                lblUpr = new Label() { Location = new Point(100, 62), AutoSize = true }; cmbUpr = CreateCmb(app.VkUpr, 200, 60);
                lblLwr = new Label() { Location = new Point(100, 92), AutoSize = true }; cmbLwr = CreateCmb(app.VkLwr, 200, 90);
                lblTtl = new Label() { Location = new Point(100, 122), AutoSize = true }; cmbTtl = CreateCmb(app.VkTtl, 200, 120);
                grpHotkeys.Controls.AddRange(new Control[] { chkCtrl, chkShift, chkAlt, lblInv, cmbInv, lblUpr, cmbUpr, lblLwr, cmbLwr, lblTtl, cmbTtl }); this.Controls.Add(grpHotkeys);

                grpOptions = new GroupBox() { Location = new Point(15, 175), Size = new Size(375, 95) };
                ...
                picLogo = new PictureBox() { Size = new Size(80, 80), Location = new Point(15, 285), Image = GenerateOoImage(80), SizeMode = PictureBoxSizeMode.Zoom };
                lblVerAuth = new Label() { Location = new Point(105, 300), AutoSize = true, Font = new Font(this.Font, FontStyle.Italic) };
                lnkGit = new LinkLabel() { Text = "github.com/didthislv", Location = new Point(105, 335), AutoSize = true };
                lnkGit.LinkClicked += (s, ev) => Process.Start("https://github.com/didthislv");

                lblSupport = new Label() { Location = new Point(250, 293), AutoSize = true, Font = new Font(this.Font, FontStyle.Bold) };
                btnCoffee = new Button() { Text = "☕ Buy me a coffee", Location = new Point(250, 318), Size = new Size(140, 32), BackColor = Color.FromArgb(255, 221, 0), ForeColor = Color.Black, Font = new Font("Segoe UI", 9, FontStyle.Bold), Cursor = Cursors.Hand };
                btnCoffee.Click += (s, ev) => Process.Start("https://www.buymeacoffee.com/didthislv");

                btnSave = new Button() { Location = new Point(165, 420), Width = 90, Height = 35 };
            ...
            private void UpdateTexts() { this.Text = Strings.SettingsTitle(app.Lang); grpHotkeys.Text = Strings.GrpHotkeys(app.Lang); grpOptions.Text = Strings.GrpOptions(app.Lang); lblInv.Text = Strings.LblInv(app.Lang); lblUpr.Text = Strings.LblUpr(app.Lang); lblLwr.Text = Strings.LblLwr(app.Lang); lblTtl.Text = Strings.LblTtl(app.Lang); chkAuto.Text = Strings.ChkAuto(app.Lang); chkSound.Text = Strings.ChkSnd(app.Lang); lblLang.Text = Strings.LblLang(app.Lang); lblSupport.Text = Strings.LblSupport(app.Lang); lblVerAuth.Text = Strings.VerAuth(app.Lang); btnSave.Text = Strings.BtnSave(app.Lang); }
